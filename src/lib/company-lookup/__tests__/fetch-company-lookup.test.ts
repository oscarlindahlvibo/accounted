...
describe('fetchCompanyLookup', () => {
  ...
  // Bolagsverket has no registry-hint concept (it either finds the company or
  // doesn't), so these exercise the TIC/SCB fallback leg specifically: the
  // Bolagsverket call is forced unconfigured so the hint from TIC's own 404
  // survives up through fetchCompanyLookup.
  it("carries the SCB registry hint beside the TIC handler's 404, once Bolagsverket falls through", async () => {
    fetchMock.mockImplementation((url: string) =>
      url.includes('/company-lookup/bolagsverket')
        ? jsonResponse(503, { code: 'NOT_CONFIGURED' })
        : jsonResponse(404, {
