...
function applyLookupFound(state: JourneyState, lookup: CompanyLookupResult): JourneyState {
  ...
  const settings: Partial<CompanySettings> = {
    ...
    // fTax is null (not false) when the provider does not report F-skatt at
    // all (Bolagsverket's VärdefullaDatamängder has no F-skatt/moms field),
    // as distinct from a confirmed false. Falling back to the existing
    // value keeps that distinction instead of silently writing null/false
