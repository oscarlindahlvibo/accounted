-- Preserve invoice distinctions and expose persisted warnings in the result.
      ...
      IF jsonb_array_length(r->'items')>0 THEN
        ...
        ELSE
          ...
          IF NOT EXISTS(SELECT 1 FROM supplier_invoice_items WHERE supplier_invoice_id=target) THEN
            IF NOT COALESCE((r->'warnings'->>'vatUnresolved')::boolean,false) THEN
...
RETURNS TABLE(resource text,total bigint,imported bigint,completed bigint,skipped bigint,needs_attention bigint,pending bigint,
  fx_unresolved bigint,vat_unresolved bigint,credit_notes_unlinked bigint)
LANGUAGE sql SECURITY INVOKER SET search_path=public AS $$
  SELECT resource,count(*),count(*) FILTER(WHERE target_id IS NOT NULL),count(*) FILTER(WHERE state='done'),
    ...
    count(*) FILTER(WHERE receipt->'warnings'->>'fxUnresolved'='true'),
    count(*) FILTER(WHERE receipt->'warnings'->>'vatUnresolved'='true'),
    count(*) FILTER(WHERE receipt->'warnings'->>'creditNoteUnlinked'='true')
