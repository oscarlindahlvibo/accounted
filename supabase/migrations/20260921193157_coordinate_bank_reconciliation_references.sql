-- bank:<uuid> references are text, so no foreign key closes the interval
-- between a retirement dependency check and a new sign-off or attachment.
-- Join the company protocol before validating the current cash row. Existing
-- historical records may still be reopened/marked removed without rebinding.
CREATE FUNCTION public.guard_bank_reconciliation_reference()
...
CREATE TRIGGER bank_reconciliation_reference_guard BEFORE INSERT OR UPDATE OF company_id, account_key
  ON public.account_reconciliations FOR EACH ROW EXECUTE FUNCTION public.guard_bank_reconciliation_reference();
CREATE TRIGGER bank_reconciliation_reference_guard BEFORE INSERT OR UPDATE OF company_id, account_key
  ON public.account_reconciliation_attachments FOR EACH ROW EXECUTE FUNCTION public.guard_bank_reconciliation_reference();
REVOKE ALL ON FUNCTION public.guard_bank_reconciliation_reference() FROM PUBLIC, anon, authenticated;
